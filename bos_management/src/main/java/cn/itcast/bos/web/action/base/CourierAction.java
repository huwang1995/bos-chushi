		...
		Specification<Courier> specification = new Specification<Courier>() {
			...
			public Predicate toPredicate(Root<Courier> root,
				...
				if (StringUtils.isNotBlank(courier.getCourierNum())) {
					/*
					 *  root.get("courierNum") ------ >C_COURIER_NUM
					 * 	courier.getCourierNum() -----> 获取courierNum的属性值
					 * cb对象转换为Sql语句 ------ > select * from 表名 where 1 = 1 
					 *      cb.equals | cb.like ... ---- >  and 字段名 + cb后面的方法名 + 属性值 
					 */
					
					Predicate p1 = 
							//转换为的sql语句为： and C_COURIER_NUM = courier.getCourierNum()
							cb.equal(
				...
				if (StringUtils.isNotBlank(courier.getCompany())) {
					//等价于 and C_COMPANY like "%" + courier.getCompany() + "%"
				 ...
				 * 
				 */
				//实体类Courier 关联 实体类中属性为standard的实体类 ，两者进行的  内连接
				// T_COURIER c inner join T_STANDARD s ON c.C_STANDARD_ID = s.id 
